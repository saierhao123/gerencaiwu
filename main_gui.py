...
class ProcessingThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.progress_updated.emit("生成可视化/报告...")
...
class MainWindow(QMainWindow):
    ...
    def update_transaction_table(self):
        ...
        for i, (_, row) in enumerate(self.df.iterrows()):
            self.transaction_table.setItem(i, 0, QTableWidgetItem(str(row.get('交易时间', ''))))
            self.transaction_table.setItem(i, 1, QTableWidgetItem(str(row.get('平台', ''))))
            self.transaction_table.setItem(i, 2, QTableWidgetItem(str(row.get('交易对方', ''))))
            self.transaction_table.setItem(i, 3, QTableWidgetItem(str(row.get('商品说明', ''))))
            amount = row.get('金额', 0) or 0
            try:
                amount = float(amount)
            except Exception:
                amount = 0.0
            self.transaction_table.setItem(i, 4, QTableWidgetItem(f"¥{amount:.2f}"))
            self.transaction_table.setItem(i, 5, QTableWidgetItem(str(row.get('交易分类', ''))))
            self.transaction_table.setItem(i, 6, QTableWidgetItem(str(row.get('支付方式', ''))))
            self.transaction_table.setItem(i, 7, QTableWidgetItem(str(row.get('交易状态', ''))))
            self.transaction_table.setItem(i, 8, QTableWidgetItem(str(row.get('交易单号', ''))))
            self.transaction_table.setItem(i, 9, QTableWidgetItem(str(row.get('调整后分类', ''))))
            self.transaction_table.setItem(i, 10, QTableWidgetItem(str(row.get('调整后子分类', ''))))
            self.transaction_table.setItem(i, 11, QTableWidgetItem(str(row.get('备注', ''))))
        
        # 设置列宽
        self.transaction_table.setColumnWidth(0, 150)  # 交易时间
        self.transaction_table.setColumnWidth(1, 80)   # 平台
        self.transaction_table.setColumnWidth(2, 120)  # 交易对方
        self.transaction_table.setColumnWidth(3, 200)  # 商品说明
        self.transaction_table.setColumnWidth(4, 100)  # 金额
        self.transaction_table.setColumnWidth(5, 100)  # 分类
        self.transaction_table.setColumnWidth(6, 100)  # 支付方式
        self.transaction_table.setColumnWidth(7, 100)  # 交易状态
        self.transaction_table.setColumnWidth(8, 150)  # 交易单号
        self.transaction_table.setColumnWidth(9, 120)  # 调整后分类
        self.transaction_table.setColumnWidth(10, 120) # 调整后子分类
        self.transaction_table.setColumnWidth(11, 150) # 备注
    ...
    def show_details_dialog(self, title: str, df: pd.DataFrame):
        ...
        v = QVBoxLayout(dlg)
        table = QTableWidget()
        cols = ["交易时间", "平台", "交易对方", "商品说明", "金额", "分类", "支付方式", "交易状态", "交易单号", "调整后分类", "调整后子分类", "备注"]
